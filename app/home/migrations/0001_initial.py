# Generated by Django 2.1.3 on 2018-11-24 07:09

from django.db import migrations, models
import django.db.models.deletion


class Migration(migrations.Migration):

    initial = True

    dependencies = [
    ]

    operations = [
        migrations.CreateModel(
            name='Amenity',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('value', models.CharField(max_length=50)),
                ('help_text', models.CharField(max_length=100)),
                ('key', models.CharField(max_length=50)),
            ],
        ),
        migrations.CreateModel(
            name='HostThumbnailImg',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('host_thumbnail_url', models.ImageField(blank=True, null=True, upload_to='pictures/host/')),
                ('host_thumbnail_url_small', models.ImageField(blank=True, null=True, upload_to='pictures/host/')),
            ],
        ),
        migrations.CreateModel(
            name='HouseImg',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('url', models.URLField()),
            ],
        ),
        migrations.CreateModel(
            name='HouseInfo',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('home_info_1', models.TextField()),
                ('home_info_2', models.TextField()),
                ('home_info_3', models.TextField()),
                ('home_info_4', models.TextField()),
                ('home_info_5', models.TextField()),
            ],
        ),
        migrations.CreateModel(
            name='Room',
            fields=[
                ('id', models.AutoField(auto_created=True, primary_key=True, serialize=False, verbose_name='ID')),
                ('bathrooms', models.IntegerField()),
                ('bedrooms', models.IntegerField()),
                ('beds', models.IntegerField()),
                ('person_capacity', models.IntegerField()),
                ('room_name', models.CharField(max_length=100)),
                ('room_type', models.CharField(max_length=20)),
                ('room_and_property_type', models.CharField(max_length=20)),
                ('public_address', models.CharField(max_length=20)),
                ('city', models.CharField(max_length=50)),
                ('price', models.IntegerField()),
                ('lat', models.FloatField()),
                ('lng', models.FloatField()),
                ('amenities', models.ManyToManyField(blank=True, null=True, to='home.Amenity')),
            ],
        ),
        migrations.AddField(
            model_name='houseinfo',
            name='room',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='room_infos', to='home.Room'),
        ),
        migrations.AddField(
            model_name='houseimg',
            name='room',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='room_house_imgs', to='home.Room'),
        ),
        migrations.AddField(
            model_name='hostthumbnailimg',
            name='room',
            field=models.ForeignKey(on_delete=django.db.models.deletion.CASCADE, related_name='host_thumbnails', to='home.Room'),
        ),
    ]
